...
import { agents as RAW_AGENTS, BOND_THRESHOLDS, getBondProgressInLevel, getBondStarCount } from "@/data/agents";
...
const AgentArchive = () => {
  ...
  const progressInLevel = getBondProgressInLevel(currentLevel, totalTurns);
      ...
      <div className="px-4">
        <div className="rounded-2xl bg-card p-4 shadow-card">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right"><p className="text-sm font-semibold text-gold-light">{getBondLabel(t, currentLevel)}</p><p className="text-[10px] text-muted-foreground">{t("archive.conversations", { n: totalTurns })}</p></div>
          </div>
          {nextThreshold && (<div className="mt-3">
            <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1"><span>{getBondLabel(t, currentLevel)}</span><span>{t("archive.next", { label: getBondLabel(t, currentLevel + 1), n: nextThreshold })}</span></div>
          ...
          {!nextThreshold && currentLevel >= BOND_THRESHOLDS.length && (<p className="mt-3 text-[11px] text-gold-light flex items-center gap-1"><Sparkles className="h-3 w-3" /> {t("archive.maxBond")}</p>)}
        </div>
      </div>
      <div className="mt-6 px-4">
        <h3 className="mb-3 font-display text-sm font-semibold text-foreground flex items-center gap-2">{t("archive.storyFragments")} <span className="text-[10px] font-normal text-muted-foreground">{t("archive.unlocked", { cur: currentLevel, total: agent.lore.length })}</span></h3>
            ...
            <motion.div key={loreEntry.level} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }} className={`rounded-2xl border p-4 transition-all ${isUnlocked ? "border-secondary/20 bg-card shadow-card" : "border-border bg-muted/30"}`}>
              <div className="flex items-center gap-2 mb-2">
                <div className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium ${isUnlocked ? "bg-gold/10 text-gold-light" : "bg-muted text-muted-foreground"}`}>
                  {isUnlocked ? <Unlock className="h-2.5 w-2.5" /> : <Lock className="h-2.5 w-2.5" />} {t("archive.lvLabel", { lv: loreEntry.level, label: getBondLabel(t, loreEntry.level) })}
                </div>
        ...
        <div className="space-y-3">
          {agent.easterEggs.map((egg, index) => { const isFound = eggsFound.includes(egg.trigger); return (
            <motion.div key={index} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + index * 0.1 }} className={`rounded-2xl border p-4 ${isFound ? "border-secondary/20 bg-card shadow-card" : "border-border bg-muted/30"}`}>
              {isFound ? (<><div className="flex items-center gap-2 mb-2"><span className="rounded-full bg-gold/10 px-2 py-0.5 text-[10px] font-medium text-gold-light flex items-center gap-1"><Sparkles className="h-2.5 w-2.5" /> {t("archive.unlockedTag")}</span><span className="text-[10px] text-muted-foreground">{t("archive.trigger", { t: egg.trigger })}</span></div><p className="text-[11px] leading-relaxed text-muted-foreground line-clamp-3 italic">{egg.response.replace("【🔮 隐藏记忆解锁】", "").trim().slice(0, 120)}…</p></>) : (

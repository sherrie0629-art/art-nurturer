...
type Bank = Record<string, { zh: PUEntry; en: PUEntry }>;
...
export function getFallbackParallelUniverse(mbtiType: string, locale: string): PUEntry {
  ...
  const loc = locale === "zh" ? entry.zh : entry.en;
  // Strip any accidental extra fields (e.g. ISTP zh "command")
  return {
    magic: { role: loc.magic.role, description: loc.magic.description },
    cyberpunk: { role: loc.cyberpunk.role, description: loc.cyberpunk.description },
  };
}